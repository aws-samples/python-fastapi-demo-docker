...
@app.post("/books")
def create_book_api(title: str = Form(...), author: str = Form(...), description: str = Form(...), db: Session = Depends(get_db)):
...
def update_book(request: Request, book_id: int, title: str = Form(...), author: str = Form(...), description: str = Form(...), db: Session = Depends(get_db)):
    book = get_book(db, book_id)
    book.title = title
    book.author = author
